<script>
  let { currentPage = '', cartItemCount = 0 } = $props();
  ...
  <div class="container mx-auto px-4">
    <nav class="flex items-center justify-between h-16">
      ...
      <div class="flex items-center space-x-4">
        ...
        <a
          href="/cart"
          class="relative p-2 text-primos-blue-100 hover:text-white transition-colors duration-200"
        ...
        >
          ...
          {#if cartItemCount > 0}
            <span
              class="absolute -top-1 -right-1 bg-primos-red-600 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center"
            >
              {cartItemCount}
            </span>
          {/if}
